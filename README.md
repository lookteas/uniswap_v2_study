...
## 📅 Day 3：Pair 合约（上）- 状态变量、mint 与 burn
...
3. 为什么 mint 前要先转入代币？
4. 手算：持有 10% LP Token，池子有 100 ETH + 200000 USDC，能取回多少？

---

## 📅 Day 4：Pair 合约（下）- swap、预言机与协议费

| 时间 | 内容                         | 文件                                                         |
| ---- | ---------------------------- | ------------------------------------------------------------ |
| 1h   | 深入理解 `swap()` 函数       | [v2-core/contracts/UniswapV2Pair.sol](cci:7://file:///d:/code/web3/uniswap/uniswap_v2/src/v2-core/contracts/UniswapV2Pair.sol:0:0-0:0) |
| 0.5h | 理解 Flash Swap 机制         | 同上                                                         |
| 1h   | 理解 `_update()` 和价格累计  | 同上                                                         |
| 0.5h | 理解 `_mintFee()` 协议费机制 | 同上                                                         |
...
**笔记验证问题**：
1. 手算：用 1 ETH 换 USDC，能得到多少？（假设池子 100:200000）
2. Flash Swap 和 Aave 闪电贷有什么区别？
3. 如何用 priceCumulative 计算过去 1 小时的平均价格？
4. 为什么协议费是 1/6 而不是直接收取？
5. 什么情况下需要调用 sync()？

---

## 📅 Day 5：Router 与 Library
...
| 1h   | 分析 Uniswap V2 常见攻击向量                                 | 安全分析                                                     |
| 1h   | 阅读经典攻击案例（三明治攻击、预言机操纵）                   | 案例学习                                                     |

**学习重点**：
- [ ] Oracle Library 的使用方法
- [ ] TWAP 预言机的优缺点
...
**笔记验证问题**：
1. 如何利用 Oracle Library 获取安全的价格？
2. 三明治攻击如何获利？如何防范？
3. 为什么直接使用 `getReserves()` 作为价格是危险的？
4. Flash Swap 可以被用于哪些攻击场景？
5. TWAP 预言机相比 Chainlink 有什么优缺点？
